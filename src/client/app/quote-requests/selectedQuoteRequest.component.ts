      ...
      .subscribe(
        quoteRequests => {
          ...
          this.productService.getProductAndAlternatives(this.quoteRequest.product_id)
            .subscribe(
              products => {
                let quoteProducts = products.map(product => new QuoteProduct(1, product.id, product.name, product.sku, product.sage_sku, this.quoteRequest.quantity, true, product.origination_price, product.prices));
